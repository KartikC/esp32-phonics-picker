...
static const struct _coeff_div coeff_div[] = {
    ...
    /* Current esp_codec_dev uses DAC OSR 0x20 at 16 kHz for smoother reconstruction. */
    {4096000, 16000, 0x01, 0x00, 0x01, 0x01, 0x00, 0x00, 0xff, 0x04, 0x10, 0x20},
...
esp_err_t es8311_init(es8311_handle_t dev, const es8311_clock_config_t *const clk_cfg, const es8311_resolution_t res_in, const es8311_resolution_t res_out)
{
    ...
    /* Reset first because the codec stays powered across USB/JTAG app flashes. */
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_RESET_REG00, 0x1F), TAG, "I2C read/write error");
    vTaskDelay(pdMS_TO_TICKS(5));

    /*
     * Match the current esp_codec_dev ES8311 bring-up used by the V2 BSP.
     * In particular, REG44=0x58 selects the internal ADCL/DACR reference path.
     * The older Arduino example omitted this and several current system-register
     * defaults, so keep the V2 path byte-for-byte aligned with the managed driver.
     */
    ...
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_CLK_MANAGER_REG03, 0x10), TAG, "I2C read/write error");
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_ADC_REG16, 0x24), TAG, "I2C read/write error");
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_CLK_MANAGER_REG04, 0x10), TAG, "I2C read/write error");
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_CLK_MANAGER_REG05, 0x00), TAG, "I2C read/write error");
    ...
    /* Start the DAC path using the same register state as esp_codec_dev. */
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_ADC_REG17, 0xBF), TAG, "I2C read/write error");
    ...
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_SYSTEM_REG14, 0x1A), TAG, "I2C read/write error");
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_SYSTEM_REG0D, 0x01), TAG, "I2C read/write error");
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_ADC_REG15, 0x40), TAG, "I2C read/write error");
...
esp_err_t es8311_voice_volume_register_set(es8311_handle_t dev, uint8_t value)
{
    return es8311_write_reg(dev, ES8311_DAC_REG32, value);
}
